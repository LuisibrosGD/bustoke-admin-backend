...
async def create_viaje(client: AsyncClient, token: str) -> int:
    body = {
        "idRuta": 1,
        "idBus": 1,
        "fechaHoraSalida": "2026-08-10T08:00:00",
        "fechaHoraLlegada": "2026-08-10T12:00:00",
        "estado": "programado",
    }
...
async def create_pasajero(client: AsyncClient, token: str, doc: str) -> int:
    body = {
        "idTipoDocumento": 1,
        "numeroDocumento": doc,
        "nombres": "Boleto",
...
class TestCreateBoleto:
    ...
    async def test_create_boleto(self, client: AsyncClient):
        ...
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550001")
...
class TestListBoletos:
    ...
    async def test_list_by_viaje(self, client: AsyncClient):
        token = await superadmin_token(client)
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550002")
        body = {
            "idViaje": viaje_id,
            "idPasajero": pasajero_id,
            "idAsiento": 1,
            "emailContacto": "list@test.com",
            "canal": "web",
            "codigoQr": f"QR-LIST-{viaje_id}",
            "precioFinal": 30.00,
        }
...
class TestGetBoleto:
    ...
    async def test_get_boleto_by_id(self, client: AsyncClient):
        token = await superadmin_token(client)
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550003")
        body = {
            "idViaje": viaje_id,
            "idPasajero": pasajero_id,
            "idAsiento": 1,
...
class TestUpdateBoleto:
    ...
    async def test_update_boleto(self, client: AsyncClient):
        token = await superadmin_token(client)
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550004")
        body = {
            "idViaje": viaje_id,
            "idPasajero": pasajero_id,
            "idAsiento": 1,
            "emailContacto": "update@test.com",
            "canal": "web",
            "codigoQr": f"QR-UPD-{viaje_id}",
            "precioFinal": 60.00,
        }
        create_resp = await client.post("/admin/viajes/boletos", json=body, headers={"Authorization": f"Bearer {token}"})
        boleto_id = create_resp.json()["id"]

        update_body = {"estado": "cancelado"}
        resp = await client.put(f"/admin/viajes/boletos/{boleto_id}", json=update_body, headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
...
class TestDeleteBoleto:
    ...
    async def test_delete_boleto(self, client: AsyncClient):
        token = await superadmin_token(client)
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550005")
        body = {
            "idViaje": viaje_id,
            "idPasajero": pasajero_id,
            "idAsiento": 1,
            "emailContacto": "delete@test.com",
            "canal": "web",
            "codigoQr": f"QR-DEL-{viaje_id}",
            "precioFinal": 20.00,
        }
        create_resp = await client.post("/admin/viajes/boletos", json=body, headers={"Authorization": f"Bearer {token}"})
        boleto_id = create_resp.json()["id"]
...
class TestCheckIn:
    ...
    async def test_checkin_boleto(self, client: AsyncClient):
        token = await superadmin_token(client)
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550006")
        body = {
            "idViaje": viaje_id,
            "idPasajero": pasajero_id,
            "idAsiento": 1,
            "emailContacto": "checkin@test.com",
            "canal": "web",
            "codigoQr": f"QR-CK-{viaje_id}",
            "precioFinal": 35.00,
        }
        create_resp = await client.post("/admin/viajes/boletos", json=body, headers={"Authorization": f"Bearer {token}"})
        boleto_id = create_resp.json()["id"]
...
class TestScanQR:
    ...
    async def test_scan_qr_success(self, client: AsyncClient):
        token = await superadmin_token(client)
        viaje_id = await create_viaje(client, token)
        pasajero_id = await create_pasajero(client, token, "55550007")
        body = {
            "idViaje": viaje_id,
            "idPasajero": pasajero_id,
            "idAsiento": 1,
            "emailContacto": "scan@test.com",
            "canal": "web",
            "codigoQr": f"QR-SCAN-{viaje_id}",
            "precioFinal": 45.00,
        }
        await client.post("/admin/viajes/boletos", json=body, headers={"Authorization": f"Bearer {token}"})
